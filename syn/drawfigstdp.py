...
axinsets[9].bar(2,(max(conds)-1)*100,color=cols[3]); print('AnCg var baseline LTP '+str((max(conds)-1)*100))
axinsets[9].bar(0,(max(conds_control)-1)*100,color='#000000'); print('Control baseline LTP '+str((max(conds_control)-1)*100))
...
for iimut in range(0,len(imutArray)):
  ...
  for imutcoeff in range(0,2):
    ...
    if len(conds) > 0:
      ...
      axinsets[3+iimut].bar(imutcoeff*2,(max(conds)-1)*100,color=cols[imutcoeff])
    ...
    if len(spfreqs) < 2:
      ...
      continue
    axinsets[6+iimut].bar(imutcoeff*2,spfreqs[-1]/spfreqs_control[-1]*100,color=cols[imutcoeff])
  ...
  axinsets[3+iimut].bar(1,(max(conds_control)-1)*100,color='#000000')
...
axinsets[5].bar(1,(max(conds)-1)*100,color=cols[2])
...
axinsets[5].bar(2,(max(conds)-1)*100,color=cols[3])
axarr[5].plot([x+30 for x in ISIs_control],conds_control,'k.-',mew=0.8,ms=0.8,lw=0.3,label = 'control')
axinsets[5].bar(0,(max(conds_control)-1)*100,color='#000000')
...
axinsets[8].bar(1,(spfreqs[-1])/spfreqs_control[-1]*100,color=cols[2])
print("imutCMcomb10 (VGCC PFC variants), AUC = "+str(spfreqs)+" (len="+str(len(spfreqs))+"), perc = "+str((spfreqs[-1])/spfreqs_control[-1]*100))
...
axinsets[8].bar(2,(spfreqs[-1])/spfreqs_control[-1]*100,color=cols[3])
axarr[8].plot(amps_control,spfreqs_control,'k.-',mew=0.8,ms=0.8,lw=0.3,label = 'control')
axinsets[8].bar(0,100,color='#000000')
print("imutCMcomb11 (VGCC ACC variants), AUC = "+str(spfreqs)+" (len="+str(len(spfreqs))+"), perc = "+str((spfreqs[-1])/spfreqs_control[-1]*100))
...
axinsets[10].bar(1,(max(conds)-1)*100,color=cols[2])
...
axinsets[10].bar(2,(max(conds)-1)*100,color=cols[3])
axarr[10].plot([x+30 for x in ISIs_control],conds_control,'k.-',mew=0.8,ms=0.8,lw=0.3,label = 'control')
axinsets[10].bar(0,(max(conds_control)-1)*100,color='#000000')
...
#axarr[16].legend(fontsize=5,frameon=False,bbox_to_anchor=(0.42,-0.7))
#axarr[21].legend(fontsize=5,frameon=False,bbox_to_anchor=(-0.4,0.22))
...
for iarea in [0,1]:
  ...
  for iISI in range(0,len(ISIs)):
    ...
    #pval = scipy.stats.ranksums(array([condpres[i] for i in isubjs_HC]),array([condpres[i] for i in isubjs_SCZ]))[1]
    #print(area+',ISI='+str(ISI)+',av. condpres='+str(mean(array([condpres[i] for i in isubjs_HC])))+' vs '+str(mean(array([condpres[i] for i in isubjs_SCZ])))+', pval='+str(pval))
